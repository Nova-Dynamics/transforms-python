...
def quat2rp(quat):
    return 2 * quat[1:] / quat[0]

def rp2quat(rp):
    norm = np.linalg.norm(rp)
    factor = 1 / np.sqrt(4 + norm**2)
    return factor * np.hstack([2, rp])
...
def dead_reckon_apply(x_hat, P_hat, step, j):
    ...
    Z_euc = np.zeros((2*L+1,SM+2))
    Z[0] = z.copy()
    Z_euc[0] = np.hstack([z[:2], quat2rotvec(np.array([z[2],0,0,z[3]]))[2], z[-2:]])
    ...
    # NOTE: dz (the cholesky decomp) gives us vectors with euclidean
    # dimensions, i.e. dz[2] is e[2], where e = [e0, e1, e2] are rotation
    # vector embeddings of S^3 in R^3
    print("dz")
    for s in dz:
        print(*[ "{: >9.5f}".format(_s) for _s in s])
    ...
    for i in range(L):
        ...
        #dlrQ_pos = np.array([dz[i][0], dz[i][1], 0])
        #drv = np.array([0, 0, dz[i][2]])
        #dlrQ = np.hstack([dlrQ_pos, rp2quat(drv)])
        ##dlrQ = np.array([dz[i][0], dz[i][1], 0, np.cos(dz[i][2]), 0 ,0, dz[i][2]])
        ## TODO SUSPECTED BUG
        ...
        # z is [x,y,a,d,dl,dr], dz has [x,y,e3,dl,dr]
        #
        # Convert z to euclidean
        Z_euc[1 + i] = Z_euc[0] + dz[i]
        Z_euc[1 + i + L] = Z_euc[0] - dz[i]

    print(Z_euc.shape)
    for item in Z_euc[:,2]:
        print(rotvec2quat(np.array([0.,0.,item])))
    Z_quat = np.array([rotvec2quat(np.array([0.,0.,e3])) for e3 in Z_euc[:,2]])
    Z = np.hstack([Z_euc[:,:2], Z_quat[:,:1], Z_quat[:,3:], Z_euc[:,-2:]])
    print("Z")
    for row in Z:
    ...
    while ( i < MAX and error > EPS ):
        i += 1
        e_rvecs = np.array([quat2rotvec(t.compose_quat(q_bar_inv, y)) for y in Y_quat])
        e = (e_rvecs[0]*W0 + np.sum(e_rvecs[1:]*W,axis=0))
        error = np.linalg.norm(e)
        q_bar = t.compose_quat(q_bar, rotvec2quat(e))
        q_bar_inv = t.invert_quat(q_bar)

    print(i, error)

    dY = np.hstack([Y_pos-y_pos_bar, e_rvecs[:,2:]]) 
