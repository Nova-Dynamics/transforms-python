...
def dead_reckon_apply(x_hat, P_hat, step, j):
    ...
    for i in range(L):
        ...
        dlrQ_rv = np.array([0, 0, dz[i][2]])
        dlrQ = np.hstack([dlrQ_pos, rp2quat(dlrQ_rv)])
    ...
    while ( i < MAX and error > EPS ):
        i += 1
        e_rvecs = np.array([quat2rp(t.compose_quat(q_bar_inv, y)) for y in Y_quat])
        e = (e_rvecs[0]*W0 + np.sum(e_rvecs[1:]*W,axis=0))
        error = np.linalg.norm(e)
        q_bar = t.compose_quat(q_bar, rp2quat(e))
